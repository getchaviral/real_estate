      ...
      <Container>
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-40px" }} transition={{ duration: 0.45 }}>
          <Card className="overflow-hidden border-primary/20 bg-gradient-to-br from-primary/10 via-background to-background">
            <CardContent className="grid gap-8 p-8 md:grid-cols-[1.1fr_0.9fr] md:p-10 lg:p-12">
              <div>
                <div className="inline-flex items-center gap-2 rounded-full bg-primary/10 px-3 py-1 text-sm font-medium text-primary">
                  <BadgePercent className="h-4 w-4" />
                  Finance support
                </div>
                <h2 className="mt-5 text-3xl font-semibold tracking-tight text-foreground sm:text-4xl">Secure a smarter home loan with expert guidance.</h2>
                <p className="mt-4 max-w-2xl text-lg leading-8 text-muted-foreground">Compare rates, understand eligibility, and move faster with financing specialists who simplify every step.</p>
