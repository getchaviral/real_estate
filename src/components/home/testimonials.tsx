            ...
            <AnimatePresence mode="wait">
              <motion.div key={current} initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: 0.3 }}>
                <Card className="overflow-hidden border-border/70 bg-background/80">
                  <CardContent className="grid gap-8 p-6 sm:p-8 lg:grid-cols-[0.7fr_1.3fr] lg:p-10">
                    <div className="flex flex-col items-center justify-center rounded-3xl bg-gradient-to-br from-primary/15 via-primary/5 to-background p-5 text-center">
                      <div className="flex h-16 w-16 items-center justify-center rounded-full bg-background/80 shadow-sm">
                        <Quote className="h-7 w-7 text-primary" />
                      </div>
                      <p className="mt-3 text-sm font-semibold uppercase tracking-[0.2em] text-primary">Buyer review</p>
                      ...
                      <p className="mt-3 text-sm font-medium text-foreground">{featured[current].name}</p>
                    ...
                    <div>
                      ...
                      <div className="mt-5 rounded-2xl border border-border/70 bg-muted/70 p-4 text-sm text-muted-foreground">
