...
export default function Filters({ filters, onFiltersChange, onApply }: FiltersProps) {
  ...
  const cityOptions = useMemo(() => Array.from(new Set(projects.map((project) => project.cityName))).sort(), []);
  const developerOptions = useMemo(() => developers.map((developer) => developer.name).sort(), []);
  const propertyTypeOptions = useMemo(() => Array.from(new Set(projects.flatMap((project) => project.propertyType))).sort(), []);
  const bhkOptions = useMemo(() => Array.from(new Set(projects.flatMap((project) => project.configurations.map((config) => config.type)))).sort(), []);
