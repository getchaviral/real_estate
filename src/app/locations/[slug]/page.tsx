...
function getLocation(slug: string): Location | undefined {
  return (locations as Location[]).find((location) => location.slug === slug);
}
...
export default function LocationPage({ params }: { params: { slug: string } }) {
  ...
  const locationProjects = (projects as Project[]).filter((project) =>
  ...
  const locationDevelopers = (developers as Developer[]).filter((developer) =>
    developer.cityIds.includes(location.id)
  );
    ...
    <>
      ...
      >
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {location.popularLocalities.map((locality, index) => (
            <motion.div
              key={locality.id}
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
