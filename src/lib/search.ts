...
const projects = entityStore.getProjects() as Project[];
const developers = entityStore.getDevelopers() as Developer[];
const locations = entityStore.getLocations() as Location[];
const propertyTypes = entityStore.getPropertyTypes() as PropertyType[];
...
export const bhkOptions = Array.from(
  new Set(projects.flatMap((project) => project.configurations.map((config) => config.type)))
).sort();
...
export function getAutocompleteSuggestions(query: string, request: Partial<SearchRequest> = {}): SearchSuggestion[] {
